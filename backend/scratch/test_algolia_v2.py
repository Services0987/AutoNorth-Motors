...
async def test_algolia():
    ...
    # Search for Team Ford specifically
    payload = {
        "requests": [
            {
                "indexName": "inventory",
                "params": "query=Team%20Ford&hitsPerPage=5"
            }
        ]
    }
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if resp.status_code == 200:
            ...
            for h in hits:
                ...
                print(f"Dealer: {h.get('dealer_name')}")
                print(f"Site IDs: {h.get('craft_site_ids')}")
                print(f"Pricing: list={h.get('list_price')}, special={h.get('special_price')}, regular={h.get('regular_price')}")
                print("-" * 20)
        else:
