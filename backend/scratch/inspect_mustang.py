...
async def test_algolia():
    ...
    payload = {
        "requests": [
            {
                "indexName": "inventory",
                "params": "query=Mustang&filters=craft_site_ids%3A34&hitsPerPage=5"
            }
        ]
    }
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if resp.status_code == 200:
            ...
            for h in hits:
                ...
                print(f"Photo Count: {h.get('photo_count')}")
                # Print keys related to images
                img_keys = [k for k in h.keys() if "img" in k.lower() or "photo" in k.lower() or "thumb" in k.lower() or "url" in k.lower()]
                for k in img_keys:
                    print(f"  {k}: {h.get(k)}")
                print("-" * 20)
        else:
